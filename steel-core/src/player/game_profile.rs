...
/// Checks whether a player name is valid for online login/profile lookup.
///
/// Vanilla player names are 3-16 ASCII alphanumeric or underscore characters.
#[must_use]
pub fn is_valid_player_name(name: &str) -> bool {
    (3..=16).contains(&name.len()) && name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_')
}

/// Generates the vanilla offline-mode UUID for a player name.
#[must_use]
